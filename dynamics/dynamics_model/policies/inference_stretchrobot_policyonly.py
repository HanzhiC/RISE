...
def main(cfg):
    ...
    weight_fpath = "/home/wiss/chenh/storage/group/srl/robotlearn/frozenvla+wm+vm_latest_pnp_basketball_gripper+head_fromhumanpretrained_actionmodelscratch/iter60000.ckpt"
    val_dataset = datamodule.val_dataset
    val_dataset.load_rgbd_frames = True

    # Or simply run on all validation samples:
    sample_list = val_dataset.samples
    sample_list = ["2026-03-11--17-30-21/0-227"]  # basketball
    # sample_list = [
    ...
    for sidx, sample in enumerate(sample_list):
        ...
        for i, _data_batch in tqdm(
            ...
            # np.savez(
            #     "assets/stretchrobot_PnP-Socket_actionInworld_statistics.npz",
            #     **action_meta_to_save,
            # )
            ...
            if VIEWER_TYPE == "o3d":
                ...
                if gt_action is not None:
                    vis_action_gt = DatasetUtils.visualize_6d_trajectory(
                        hand_action_gt,
                        size=0.01,
                        cmap_name="plasma",
                        to_mesh=True,
                ...
                if gt_action is not None:
                    vis_action_gt = DatasetUtils.visualize_3d_trajectory(
                        hand_action_gt[:, :3, 3],
                        size=0.01,
                        cmap_name="plasma",
                        to_mesh=False,
