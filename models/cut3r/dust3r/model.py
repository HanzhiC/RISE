...
# from models.croco import CroCoNet, CrocoConfig  # type: ignore
from croco.models.croco import CroCoNet, CrocoConfig  # type: ignore
...
class ARCroco3DStereo(CroCoNet):
    ...
    def _encode_views(self, views, img_mask=None, ray_mask=None):
        ...
        if selected_imgs.size(0) > 0:
            ...
            # img_out: [(num_views * batch_size, 196, 1024)]
            # img_pos: (num_views * batch_size, 196, 2)
        else:
        ...
        ]  # [(num_views * batch_size, 196, 1024)]
        full_pos = torch.zeros(
            len(views) * batch_size,
            *img_pos.shape[1:],
            device=img_pos.device,
            dtype=img_pos.dtype,
        )  # (num_views * batch_size, 196, 2)

        for i in range(len(img_out)):  # Several layers of image features
            full_out[i][img_masks_flat] += img_out[i]  # Update the image tokens
            full_out[i][~img_masks_flat] += self.masked_img_token  # Masked image token
        full_pos[img_masks_flat] += img_pos  # Update the position of the image tokens

        # Ray map update (use the ray feature instead of the image feature)
        if ray_maps.shape[-1] == 6:
            ray_maps = ray_maps.permute(
                0, 3, 1, 2
        ...
        if selected_ray_maps.size(0) > 0:
            ...
            for i in range(len(ray_out)):
                # View-wise update, use the ray feature instead of the image feature
            ...
            # Basically update nothing ...
            for i in range(len(ray_out)):
                full_out[i][ray_mask_flat] += ray_out[i] * 0.0
                full_out[i][~ray_mask_flat] += self.masked_ray_map_token * 0.0
        return (
            shapes.chunk(len(views), dim=0),  # (batch_size, 2), (batch_size, 2), ...
            [
                out.chunk(len(views), dim=0) for out in full_out
            ],  # [[(batch_size, 196, 1024), (batch_size, 196, 1024), ...]]
            full_pos.chunk(
                len(views), dim=0
            ),  # (batch_size, 196, 2), (batch_size, 196, 2), ...
        )

    def _decoder(self, f_state, pos_state, f_img, pos_img, f_pose, pos_pose):
        """
        This function fuses the state feature S_{t-1} and the image feature F_t, so as to acquire the fused
        feature S_t, F_t', z_t'.

        f_state: (batch_size, 768, 768)
        f_img: (batch_size, 196, 1024)
        f_pose: (batch_size, 1, 768)
        pos_state: (batch_size, 196, 2)
        pos_img: (batch_size, 196, 2)
        pos_pose: (batch_size, 1, 2) # -1
        """
        final_output = [(f_state, f_img)]  # before projection
        assert f_state.shape[-1] == self.dec_embed_dim
        f_img = self.decoder_embed(
            f_img
        )  # (batch_size, 196, 1024) -> (batch_size, 196, 768)
        if self.pose_head_flag:
            assert f_pose is not None and pos_pose is not None
            f_img = torch.cat([f_pose, f_img], dim=1)  # (batch_size, 197, 768)
            pos_img = torch.cat([pos_pose, pos_img], dim=1)  # (batch_size, 197, 2)
        final_output.append(
            (f_state, f_img)
        )  # [(batch_size, 768, 768), (batch_size, 197, 768)]
        for blk_state, blk_img in zip(
            self.dec_blocks_state, self.dec_blocks
        ):  # Several decoder blocks
            if (
    ...
    def extract_state_observation_feature_streaming(
        self,
        view,
        state_feats_cache=None,
        observation_feats_cache=None,
        shapes_cache=None,
        poss_cache=None,
        max_seq_length=1,
    ):
        shapes_curr, feat_ls_curr, poss_curr = self._encode_views(
            [view]
        )  # shape, pos, feat_ls[-1] are tuples with length num_views
        feat_curr = feat_ls_curr[-1]  # (batch_size, 196, 1024)
        assert (
            len(feat_ls_curr) == len(poss_curr) == 1 == len(shapes_curr)
        ), f"{len(feat_ls_curr)}, {len(poss_curr)}, {len(shapes_curr)}"
        is_init = False
        if state_feats_cache is None:
            is_init = True
            state_feat, state_pos = self._init_state(
                feat_curr[0], poss_curr[0]
            )  # (batch_size, 768, 768), (batch_size, 768, 2)
            mem = self.pose_retriever.mem.expand(
                feat_curr[0].shape[0], -1, -1
            )  # (batch_size, 256, 1536)
            init_state_feat = state_feat.clone()
            init_mem = mem.clone()
            state_feats_cache, observation_feats_cache, shapes_cache, poss_cache = (
                deque(maxlen=max_seq_length),
                deque(maxlen=max_seq_length),   
                deque(maxlen=max_seq_length),
                deque(maxlen=max_seq_length),
            )
            state_feats_cache.append(
                [state_feat, state_pos, init_state_feat, mem, init_mem]
            )

        state_feat, state_pos, init_state_feat, mem, init_mem = state_feats_cache[-1]

        if self.pose_head_flag:
            global_img_feat = self._get_img_level_feat(
                feat_curr[0]
            )  # Current image feature (batch_size, 1, 1024)
            if self.pose_head_flag:
                if is_init:
                    pose_feat = self.pose_token.expand(
                        feat_curr[0].shape[0], -1, -1
                    )  # (batch_size, 1, 768)
                else:
                    pose_feat = self.pose_retriever.inquire(
                        global_img_feat, mem
                    )  # (batch_size, 1, 768)
                pose_pos = -torch.ones(
                    feat_curr[0].shape[0],
                    1,
                    2,
                    device=feat_curr[0].device,
                    dtype=poss_curr[0].dtype,
                )
            else:
                pose_feat = None
                pose_pos = None

        # Update the state feature and pose feature
        ...
        out_pose_feat = dec[-1][
            :, 0:1
        ]  # (batch_size, 1, 768) -> z_t', fused pose feature
        new_mem = self.pose_retriever.update_mem(
            mem, global_img_feat, out_pose_feat
        )  # Update the pose memory
        assert len(dec) == self.dec_depth + 1
        head_input = [
            dec[
                0
            ].float(),  # (batch_size, 196, 1024); image feature at the beginning of the decoder, F_t, no fusion with the state feature
            dec[self.dec_depth * 2 // 4][
                :, 1:
            ].float(),  # (batch_size, 196, 768); Fused image feature at the 1 / 2 layer of the decoder
            dec[self.dec_depth * 3 // 4][
                :, 1:
            ].float(),  # (batch_size, 196, 768); Fused image feature at the 3 / 4 layer of the decoder
            dec[
                self.dec_depth
            ].float(),  # (batch_size, 197, 768); Fused image and pose feature at the penultimate layer of the decoder
        ]

        # Update the state memory and pose memory
        img_mask = view["img_mask"]
        update = view.get("update", None)
        ...
        state_feat = new_state_feat * update_mask + state_feat * (
            1 - update_mask
        )  # state_feat == new_state_feat == S_t
        mem = new_mem * update_mask + mem * (1 - update_mask)  # mem == new_mem
        ...
        observation_feats_cache.append(head_input)
        shapes_cache.append(shapes_curr[-1])
        poss_cache.append(poss_curr[-1])

        return state_feats_cache, observation_feats_cache, shapes_cache, poss_cache

    def extract_state_observation_feature(self, views):
        print("================================================")
        shapes, feat_ls, poss = self._encode_views(
            views
        )  # shape, pos, feat_ls[-1] are tuples with length num_views
        feat = feat_ls[-1]  # (batch_size, 196, 1024), (batch_size, 196, 1024), ...

        # Initialize the state feature and pose memory using the first frame image feature and pose feature
        state_feat, state_pos = self._init_state(
            feat[0], poss[0]
        )  # (batch_size, 768, 768), (batch_size, 768, 2)
        mem = self.pose_retriever.mem.expand(
        ...
        state_feats = []
        observation_feats = []
        state_feats.append([state_feat, state_pos, init_state_feat, mem, init_mem])

        # Iterate over the views
        for i in range(len(views)):

            feat_i = feat[i]  # (batch_size, 196, 1024)
            pos_i = poss[i]  # (batch_size, 196, 2)
            if self.pose_head_flag:
                global_img_feat_i = self._get_img_level_feat(
                    feat_i
                )  # Current image feature (batch_size, 1, 1024)
                if i == 0:
                    pose_feat_i = self.pose_token.expand(
                        feat_i.shape[0], -1, -1
                    )  # (batch_size, 1, 768)
                else:
            ...
            # print(state_feat[0, 0, :5])

            out_pose_feat_i = dec[-1][
                :, 0:1
            ]  # (batch_size, 1, 768) -> z_t', fused pose feature
            new_mem = self.pose_retriever.update_mem(
            ...
            # Print the state feature
            # print(state_feat[0, 0, :5])

            # Track the state feature and observation feature
            state_feats.append([state_feat, state_pos, init_state_feat, mem, init_mem])
            observation_feats.append(head_input)

        return state_feats, observation_feats, shapes, poss

    def predict_geometry(self, views, shapes, poss, observation_feats, state_feats):
        # assert (
        #     len(observation_feats) == len(state_feats) - 1
        # ), f"{len(observation_feats)}, {len(state_feats)}"
        ress = []
        for i in range(len(observation_feats)):
            state_feat, state_pos, init_state_feat, mem, init_mem = state_feats[i + 1]
            observation_feat_i = observation_feats[i]  # [F_t', z_t']
            res = self._downstream_head(observation_feat_i, shapes[i], pos=poss[i])
            ress.append(res)
        return ARCroco3DStereoOutput(ress=ress, views=views)

    def _forward_impl(self, views, ret_state=False):
        shape, feat_ls, pos = self._encode_views(
            views
        )  # shape, pos, feat_ls[-1] are tuples with length num_views
        feat = feat_ls[-1]  # (batch_size, 196, 1024), (batch_size, 196, 1024), ...
        ...
        for i in range(len(views)):
            ...
            if self.pose_head_flag:
                ...
                pose_pos_i = -torch.ones(
                    feat_i.shape[0], 1, 2, device=feat_i.device, dtype=pos_i.dtype
                )  # Positional embedding of the pose feature is set to -1
            else:
                pose_feat_i = None
                pose_pos_i = None
            ...
            # Done with the Feature Fusion, last item is the fused image and pose feature

            # Update the state feature and pose memory
            img_mask = views[i]["img_mask"]
            update = views[i].get("update", None)
            if update is not None:
                update_mask = (
            ...
            # Now we need to do the downstream head
            res = self._downstream_head(
                head_input, shape[i], pos=pos_i
            )  # (batch_size, 196, 1024)
            ress.append(res)
    ...
    ## Seems the following functions are not used during inference
