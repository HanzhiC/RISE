...
class Arti4DDataset:
    ...
    def __init__(
        ...
        self.tf_file_path = Path(tf_file_path) if tf_file_path is not None else None
        ...
        self.droid_slam = droid_slam
        self.calc_pcd = calc_pcd
        self.rgb_path = Path(self.root_path) / "rgb"
        self.depth_path = Path(self.root_path) / "depth"
        self.camera_poses_path = Path(self.root_path) / "odom"
    ...
    def _load_parameters(self):
        ...
        if self.camera_poses_path and self.gt_poses:
            ...
            if os.path.exists(self.root_path / "registration_matrix.json"):
                ...
                loguru.logger.warning(
                    "Registration matrix file not found. Please check if the file exists and the path is correct: %s",
                    self.root_path / "registration_matrix.json",
                )
    ...
    def _read_camera_params(self, file_path: Path):
        """
    ...
    def get_scene_pcd(self, voxel_size: float = 0.01):
        """
...
if __name__ == "__main__":
    ...
    root_path = (
        "/home/wiss/chenh/arti4d_tmp/arti4d/raw/din080/scene_2025-04-11-12-58-58"
    )
    tf_file_path = "/home/wiss/chenh/arti4d_tmp/arti4d/raw/azure_tf.json"
    dataset = Arti4DDataset(root_path=root_path, tf_file_path=tf_file_path)
    print("Dataset length:", len(dataset))

    scene_pcd = dataset.get_scene_pcd(voxel_size=0.025)
    scene_pcd_gt = o3d.io.read_point_cloud(
        Path(root_path) / "compressed_point_cloud.ply"
    ...
    o3d.visualization.draw([scene_pcd, scene_pcd_gt])
