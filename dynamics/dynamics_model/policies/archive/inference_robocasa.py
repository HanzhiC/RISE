...
from policies.robocasa_policy_wrapper import PolicyVLAWorldModelWrapperRoboCasa
DEFAULT_FPS = 15
QUERY_HORIZON = 15
EXECUTE_HORIZON = 15
...
INSTRUCTION = None
ACTION_DIM = 20
VIEWPOINT = "head"  # "gripper" or "head"


@torch.no_grad()
def main(cfg):
    # cfg.ALGORITHM.model.num_steps = 2
    cfg.DATA.load_tracks = False
    ...
    policy = PolicyVLAWorldModelWrapperRoboCasa(
        cfg,
    ...
    for sidx, sample in enumerate(sample_list):
        ...
        for i, _data_batch in tqdm(
            ...
            # Do the inference
            outputs = policy.inference(data_batch, action_only=True)
            pred_action_chunk = outputs["latest_action_chunk"].cpu().numpy()
            ...
            elif VIEWPOINT == "gripper":
                color = _data_batch["color_gripper"][0].cpu().numpy().transpose(1, 2, 0)
                depth = _data_batch["depth_gripper"][0].cpu().numpy()
                intr = _data_batch["intrinsics_gripper"][0].cpu().numpy()
