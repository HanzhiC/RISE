...
from models.cut3r.dust3r.heads.postprocess import (
    postprocess,
    postprocess_desc,
    postprocess_rgb,
...
import models.cut3r.dust3r.utils.path_to_croco  # noqa
from models.cut3r.dust3r.blocks import Mlp  # noqa
from models.cut3r.dust3r.utils.geometry import geotrf
from models.cut3r.dust3r.utils.camera import pose_encoding_to_camera, PoseDecoder
from models.cut3r.dust3r.blocks import ConditionModulationBlock
...
class LinearPts3dPose(nn.Module):
    ...
    def forward(self, decout, img_shape, **kwargs):
        H, W = img_shape
        tokens = decout[-1] # Last layer feature
        if self.has_pose:
            pose_token = tokens[:, 0] # (batch_size, 768)
            tokens = tokens[:, 1:] # (batch_size, 196, 768)
            with torch.cuda.amp.autocast(enabled=False):
                pose = self.pose_head(pose_token) # A head to predict the pose
            cross_tokens = tokens 
        ...
        with torch.cuda.amp.autocast(enabled=False):
            B, S, D = tokens.shape
            feat = self.proj(tokens)  # B,196,1024
            feat = feat.transpose(-1, -2).view(
                B, -1, H // self.patch_size, W // self.patch_size
            ) # B, 1024, 14, 14
            feat = F.pixel_shuffle(feat, self.patch_size)  # B, 4, 224, 224
            final_output = postprocess(
                feat, self.depth_mode, self.conf_mode, pos_z=True
            ) 
            final_output["pts3d_in_self_view"] = final_output.pop("pts3d") # Measured in the current frame
            final_output["conf_self"] = final_output.pop("conf")
            ...
            if self.has_pose:
                ...
                final_output["pts3d_in_other_view"] = tmp.pop("pts3d") # Measured in the initial frame
