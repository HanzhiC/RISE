...
def main(cfg):
    ...
    cfg.DATA.data_dir = "/home/chenh/hanzhi_ws/egoasis_dataset/stretch_demo_data/"
    datamodule = datamodule_factory(cfg)
    datamodule.setup()
    # weight_fpath = os.path.join(cfg.log_dir, cfg.name, "last.ckpt")
    val_dataset = datamodule.val_dataset
    weight_ckpt = "../egoasis_dataset/vla_stretch_worldaction_horizon8_smallnet/iter28000.ckpt"

    # Or simply run on all validation samples:
    sample_list = val_dataset.samples
    sample_list = [
        "2026-01-13--16-01-01/0-51",
    ]
    ...
    for sidx, sample in enumerate(sample_list):
        ...
        for i, _data_batch in tqdm(
            ...
            pred_progress = pred_action_chunk[..., -1] * (len(video_data) - 1)
            pred_action_chunk = pred_action_chunk[..., :ACTION_DIM]
            print(f" ===> Current progress: {pred_progress=}")
