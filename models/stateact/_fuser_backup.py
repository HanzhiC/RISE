...
class MultiModalFusionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.virtual_future_state_tokens = nn.Parameter(torch.randn(1, 1, 1, dim))
        self.virtual_future_action_tokens = nn.Parameter(
            torch.randn(1, self.n_virtual_future, dim)
        )
    ...
    def forward(self, x_obs, x_action, x_language):
        ...
        for i in range(len(self.temporal_state2action_blocks)):
            ...
            temporal_state_tokens = state_tokens.contiguous().view(
                B * N, self.horizon_past + 1, -1
            )  # B N H C -> (B N) H C
            temporal_action_tokens = action_tokens  # [B, H, C]
            temporal_state_tokens = self.temporal_state_blocks[i](temporal_state_tokens)
            temporal_action_tokens = self.temporal_action_blocks[i](
                temporal_action_tokens
            )
            ...
            temporal_state_tokens = self.temporal_state2action_blocks[i](
                temporal_state_tokens, temporal_action_tokens
            )
            temporal_action_tokens = self.temporal_action2state_blocks[i](
                temporal_action_tokens, temporal_state_tokens
            )
            ...
            action_tokens = self.temporal_action2lang_blocks[i](
            ...
            spatial_state_tokens = (
                ...
                .contiguous()
                .view(B * (self.horizon_past + 1), N, -1)
            )  # B N T C -> (B T) N C
            ...
            virtual_register_state_tokens = self.spatial_virtual2lang_blocks[j](
...
if __name__ == "__main__":
    state_dim, action_dim, dim, horizon = 4, 6, 384, 15
    B, N, H, K, D, L = 1, 196, horizon, 15, dim, 53
    N_query_points = 1024
    N_query_states = 4
    visual_size = 14
