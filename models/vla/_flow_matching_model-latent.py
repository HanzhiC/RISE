...
class VLAFlowMatching(nn.Module):
    def __init__(self, config: dict):
        ...
        self.value_visual_feature_type = config.get("value_visual_feature_type", "dinov3")
        self.use_video_latent_visual_feature = self.value_visual_feature_type in [
            "video_latent",
            "wan_vae",
            "wan_vae_sliding",
        ]
        self.value_visual_feature_dim = config.get(
            "value_visual_feature_dim",
            16 if self.use_video_latent_visual_feature else 768,
        )
        self.value_visual_num_tokens = config.get(
            "value_visual_num_tokens",
            256 if self.use_video_latent_visual_feature else 196,
        )
        self.value_visual_context_length = (
            self.visual_horizon * self.value_visual_num_tokens
        )
        self.value_visual_feature_output_scale = config.get(
            "value_visual_feature_output_scale",
            1.0 if self.use_video_latent_visual_feature else 5.0,
        )
        ...
        if self.use_wm:
            self.visual_proj_for_wm = nn.Linear(
                self.value_visual_feature_dim + 6, self.feature_dim
            )
            ...
            self.dynamics_visual_dim = (
                self.value_visual_feature_dim if self.dynamics_predict_visual else 0
            )
            ...
            if self.dynamics_predict_visual:
                self.dynamics_transition_dim += self.dynamics_visual_dim
            ...
            dynamics_model_kwargs = dict(
                ...
                context_length=self.value_visual_context_length + 30,
                dtype=self.dtype,
                action_chunk_size=self.action_chunk_size,
                language_token_size=30,
            )
        ...
        if self.use_vm:
            ...
            self.dynamics_proj_for_vm = None
            # if self.am_use_gripper_image:
            ...
            vm_model_kwargs = dict(
                ...
                context_length=(
                    self.value_visual_context_length + 30 + self.action_chunk_size
                ),
                dtype=self.dtype,
                n_cond_layers=1,
                language_token_size=30,
        ...
        for component in model_components:
            ...
            if component == "action_model":
                ...
                for _, param in self.language_proj_for_vm.named_parameters():
                    param.requires_grad = False
                if self.dynamics_proj_for_vm is not None:
    ...
    def set_dynamics_model(self, fm_model):
        ...
        self.value_visual_feature_type = fm_model.value_visual_feature_type
        self.use_video_latent_visual_feature = fm_model.use_video_latent_visual_feature
        self.value_visual_feature_dim = fm_model.value_visual_feature_dim
        self.value_visual_feature_output_scale = (
            fm_model.value_visual_feature_output_scale
        )
    ...
    def velocity_loss(
        ...
        values_future=None,
        values_meta=None,
        image_features_wm=None,
        image_features_vm=None,
        ...
        history_raymaps_wm=None,
        history_raymaps_vm=None,
        ...
        if self.use_wm:
            if image_features_wm is None:
                image_features_wm = image_features
            if history_raymaps_wm is None:
                history_raymaps_wm = history_raymaps
            prefix_image_dynamics, prefix_language_dynamics = (
                self.prepare_vision_language_features(
                    model="wm",
                    image_features=image_features_wm,
                    language_features=language_features,
                    history_raymaps=history_raymaps_wm,
                    image_features_gripper=image_features_gripper,
                    history_raymaps_gripper=history_raymaps_gripper,
                )
            )
            ...
            x_t_dynamics, c_dynamics = self.dynamics_model(
                x=x_t_dynamics,
                x_history=torch.zeros_like(history_state_dynamics),
        ...
        if self.use_vm:
            ...
            if image_features_vm is None:
                image_features_vm = image_features
            if history_raymaps_vm is None:
                history_raymaps_vm = history_raymaps
            ...
            prefix_image_value, prefix_language_value = (
                self.prepare_vision_language_features(
                    ...
                    language_features=language_features,
                    history_raymaps=history_raymaps_vm,
                )
            )
            ...
            if (
                ...
                res_visual = int(image_features_vm.shape[-2] ** 0.5)
                ...
                with torch.no_grad():
                    ...
                    future_v, future_c_values = self.value_model(
                        ...
                        context=future_prefix_image_value_gt,
                ...
                future_v_pred, future_c_values_pred = self.value_model(
                    ...
                    context=future_prefix_image_value_pred,
    ...
    def dynamics_visual_to_vm_image_features(
        ...
        return (
            x[:, None].expand(-1, self.horizon, -1, -1)
            * self.value_visual_feature_output_scale
        )
    ...
    def preprocess_history_visual_features(
        ...
        target_visual_horizon: Optional[int] = None,
    ):
        """
        Downsample the history, then repeat along T so output length is self.horizon.
        ...
        if target_visual_horizon is None:
            target_visual_horizon = self.visual_horizon
        sample_horizon = min(self.history_visual_horizon, target_visual_horizon)

        # Downsample: e.g. Bx15x..., sample_horizon=5 -> indices [2, 5, 8, 11, 14]
        if sample_mode == "uniform":
            if sample_horizon <= 1:
                sample_indices = [self.horizon - 1]
            else:
                step = (self.horizon - 1) // (sample_horizon - 1)
                start = (self.horizon - 1) - step * (sample_horizon - 1)
                sample_indices = [start + i * step for i in range(sample_horizon)]
        elif sample_mode == "latest":
            sample_indices = list(
                np.arange(
                    self.horizon - 1,
                    self.horizon - sample_horizon - 1,
                    -1,
                )
            )[::-1]
        else:
            raise ValueError(f"Invalid sample mode: {sample_mode}")
        history = history[:, sample_indices]  # [B, sample_horizon, ...]

        # Repeat each sampled step to the requested visual context length.
        repeat_count = max(target_visual_horizon // sample_horizon, 1)
        history = history.repeat_interleave(repeat_count, dim=1)
        if history.size(1) < target_visual_horizon:
            pad_len = target_visual_horizon - history.size(1)
            rest_of_the_shape = tuple(history.shape[2:])
            history = torch.cat(
                [history, history[:, -1:, ...].expand(-1, pad_len, *rest_of_the_shape)],
                dim=1,
            )
        elif history.size(1) > target_visual_horizon:
            history = history[:, -target_visual_horizon:]
        return history

    def compute_losses(self, data_batch: dict):
        action_rel = "_rel" if self.action_in_relative else ""
        image_features = data_batch.get(
            "history_visual_feature_patch_dinov3",
            data_batch["history_visual_feature_patch"],
        )  # Policy visual features: [B, H, P, C]
        image_features_vm = data_batch["history_visual_feature_patch"]
        ...
        history_raymaps_model = data_batch.get(
            "history_raymap_video_latent", history_raymaps
        )
        ...
        image_features_next = data_batch.get("history_visual_feature_patch_next", None)
        history_raymaps_next = data_batch.get(
            "history_raymap_next_video_latent",
            data_batch.get("history_raymap_next", None),
        )
        ...
        losses = self.velocity_loss(
            ...
            image_features_wm=image_features_vm,
            image_features_vm=image_features_vm,
            # values_future=values_future,
            ...
            history_raymaps=history_raymaps,
            history_raymaps_wm=history_raymaps_model,
            history_raymaps_vm=history_raymaps_model,
            history_raymaps_gripper=history_raymaps_gripper,
    ...
    def get_aux_info(self, data_batch: dict):
        ...
        if (
            ...
            visual_feature_for_null = data_batch.get(
                "history_visual_feature_patch_dinov3",
                data_batch["history_visual_feature_patch"],
            )
            aux_info["history_visual_feature_patch_null"] = (
                visual_feature_for_null.clone().fill_(1e-3)
            )
    ...
    def embed_vision_language_features(
        ...
        image_features = self.preprocess_history_visual_features(
            ...
            target_visual_horizon=target_visual_horizon,
        )
    ...
    def forward(
        ...
        outputs = OrderedDict()
        image_features_model = data_batch["history_visual_feature_patch"]
        image_features = data_batch.get(
            "history_visual_feature_patch_dinov3", image_features_model
        )  # Policy visual features: [B, H, P, C]
        language_features = data_batch["language_feature"]  # [B, L, 768]
        history_raymaps = data_batch["history_raymap"]
        history_raymaps_model = data_batch.get(
        ...
        if not value_only:
            ...
            image_features_model = TensorUtils.repeat_by_expand_at(
                image_features_model, repeats=num_samples, dim=0
            )
            ...
            history_raymaps_model = TensorUtils.repeat_by_expand_at(
                history_raymaps_model, repeats=num_samples, dim=0
            )
            ...
            actions_scaled, guidance_losses_actions = self.sample_actions(
                ...
                initial_dynamics_feature=initial_dynamics_feature,
                history_state_dynamics=history_state_dynamics,
                image_features_dynamics=image_features_model,
                history_raymaps_dynamics=history_raymaps_model,
                drop_action_mask=None,
            )
            ...
            if self.use_wm and not action_only:
                dynamics_scaled = self.sample_dynamics(
                    image_features=image_features_model,
                    language_features=language_features,
                    history_raymaps=history_raymaps_model,
                    input_actions=(
            ...
            if self.use_wm and not action_only:
                ...
                if self.dynamics_predict_visual:
                    ...
                    dynamics_visual = (
                        dynamics_visual * self.value_visual_feature_output_scale
                    )
    ...
    def sample_actions(
        ...
        image_features_dynamics: torch.Tensor = None,
        history_raymaps_dynamics: torch.Tensor = None,
        drop_action_mask: torch.Tensor = None,
    ):
        ...
        bsize = image_features.shape[0]
        device = image_features.device
        if image_features_dynamics is None:
            image_features_dynamics = image_features
        if history_raymaps_dynamics is None:
            history_raymaps_dynamics = history_raymaps
        ...
        while time >= -dt / 2:
            ...
            if enable_guidance:
                ...
                x_t_dynamics_clean = self.sample_dynamics(
                    image_features=image_features_dynamics,
                    language_features=language_features,
                    history_raymaps=history_raymaps_dynamics,
    ...
    def predict_values_step(
        ...
        """Predict values from action and visual-language features."""
        with grad_withctx():
